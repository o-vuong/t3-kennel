...
export async function sendBookingStatusNotification(
  ...
  try {
    ...
    if (newStatus === BookingStatus.CANCELLED || newStatus === BookingStatus.NO_SHOW) {
    ...
    console.log(`Booking status notification sent for booking ${bookingId}: ${newStatus}`);
  } catch (error) {
    console.error(`Failed to send booking status notification for ${bookingId}:`, error);
  }
}
...
export async function sendBookingReminderNotification(
  ...
  try {
    ...
    console.log(`Booking reminder notification sent for booking ${bookingId}: ${reminderType}`);
  } catch (error) {
    console.error(`Failed to send booking reminder notification for ${bookingId}:`, error);
  }
}
